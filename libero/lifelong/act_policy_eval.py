...
@hydra.main(config_path="../configs", config_name="ACT_eval", version_base=None)
def main(hydra_cfg):
    # preprocessing
    ...
    create_experiment_dir(cfg, eval_flag=True)
    ...
    if cfg.quantize:
        replace_linear_with_qlinear(algo.policy.act_model, quantype='full', num_bits=8)
        algo.policy = algo.policy.to(cfg.device)
