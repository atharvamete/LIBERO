...
class SkillGPT_Model(BasePolicy):
    def __init__(self, cfg, shape_meta):
        ...
        self.vae_2_block_size = policy_cfg.skill_vae_2.skill_block_size
        ...
        self.factor = policy_cfg.skill_vae_1.skill_block_size // policy_cfg.skill_vae_2.skill_block_size
    ...
    def forward(self, data):
        indices_1 = self.skill_vae_1.get_indices(data["actions"]).long()
        indices_2 = self.get_indices_from_actions(data["actions"]) + int(self.prior_cfg.vocab_size_1)
        context = self.obs_encode(data)
        start_tokens = (torch.ones((context.shape[0], 1))*self.start_token).long().to(self.device)
        x = torch.cat([start_tokens, indices_1, indices_2[:,:-1]], dim=1)
        targets = [indices_1, indices_2 - int(self.prior_cfg.vocab_size_1)]
        logits_1, logits_2, prior_loss, offset = self.skill_gpt(x, context, targets, return_offset=self.return_offset)
        if self.return_offset:
            offset = offset.view(-1, self.vae_1_block_size, self.act_dim)
            with torch.no_grad():
                probs = torch.softmax(logits_2, dim=-1)
                sampled_indices = torch.multinomial(probs.view(-1,logits_2.shape[-1]),1)
                sampled_indices = sampled_indices.view(-1,logits_2.shape[1])
            pred_actions = self.get_action_from_indices(sampled_indices)
    ...
    def get_action_from_indices(self, indices):
        actions = []
        for i in range(self.factor):
            start = i*self.prior_cfg.block_size_2
            pred_actions = self.skill_vae_2.decode_actions(indices[:,start:start+self.prior_cfg.block_size_2])
            actions.append(pred_actions)
        actions = torch.cat(actions, dim=1)
        return actions

    def get_indices_from_actions(self, actions):
        indices = []
        for i in range(self.factor):
            start = i*self.vae_2_block_size
            pred_indices = self.skill_vae_2.get_indices(actions[:,start:start+self.vae_2_block_size,:])
    ...
    def get_indices_top_k(self, context):
        ...
        for i in range(self.prior_cfg.block_size_1):
            logits_1,_,_ = self.skill_gpt(x, context)
        ...
        for i in range(self.prior_cfg.block_size_2):
            if i == self.prior_cfg.block_size-1:
                ...
                _,logits_2,_ = self.skill_gpt(x, context)
            next_indices = top_k_sampling(logits_2[:,-1,:], self.prior_cfg.beam_size, self.prior_cfg.temperature) + int(self.prior_cfg.vocab_size_1)
            x = torch.cat([x, next_indices], dim=1)
        return x[:,self.prior_cfg.block_size_1:], offset
